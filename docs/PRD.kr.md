...
- **완전한 에코시스템 통합**: 7개 기존 시스템과의 네이티브 통합
...
| **database_system** | 스토리지 보안 | 매개변수화된 쿼리 빌딩을 통한 SQL 인젝션 보호 | ✅ 통합 완료 (선택 사항) |
...
다음 표는 모든 개발 단계의 통합 개요를 제공합니다. 이것은 프로젝트의 공식 단계 정의이며, 다른 모든 문서(SRS, SDS, FEATURES)는 이 정의를 참조합니다.

| 단계 | 이름 | 범위 | 상태 |
|------|------|------|------|
| **1** | 기반 구축 | DICOM 코어: 데이터 요소, 데이터 세트, 파일 I/O (Part 10), 태그 사전, 전송 구문 | ✅ 완료 |
...
| **5** | 엔터프라이즈 기능 | 전체 AWS/Azure SDK | 계획됨 |

**교차 참조**: PRD 단계는 SRS 요구사항 단계에 매핑됩니다 (SRS-CORE-* → Phase 1, SRS-NET-*/SRS-ENC-* → Phase 2, SRS-SVC-* → Phase 3, SRS-WF-*/SRS-SEC-*/SRS-WEB-* → Phase 4).
...
*참고: Phase 5에서 VTK 통합, FHIR, 클러스터링은 제거되었습니다 — VTK는 프로젝트의 "외부 DICOM 라이브러리 미사용" 철학과 충돌하고, FHIR은 pacs_bridge에서 처리되며, 클러스터링은 인프라 수준 솔루션(예: Kubernetes)으로 대체됩니다. 연결 풀링은 이전에 여기 나열되었으나, 클라이언트 모듈(`remote_node_manager`)에 이미 구현되어 있습니다 — FR-10.4 참조.*
...
모든 프로젝트 문서는 모호함을 피하기 위해 일관된 버전 관리 체계를 따릅니다.

#### 제품 버전

형식: `MAJOR.MINOR.PATCH.BUILD` (예: `0.2.0.0`)

| 구성요소 | 의미 | 증가 시점 |
|----------|------|----------|
| MAJOR | 주요 릴리스 마일스톤 | 호환성 변경 또는 프로덕션 릴리스 (0 = 프리프로덕션) |
| MINOR | 단계 완료 | 개발 단계 완료 시 |
| PATCH | 기능 추가 | 단계 내 새 기능 또는 주요 변경 |
| BUILD | 버그 수정 / 패치 | 사소한 수정, 교정, 문서 패치 |

**현재 제품 버전**: `0.2.0.0` (Phase 1-4 완료, 프리프로덕션)

#### 문서 개정 버전

각 문서는 제품 버전과 독립적으로 자체 개정 이력을 추적합니다:

형식: `MAJOR.MINOR.PATCH` (예: `2.6.0`)

| 구성요소 | 의미 |
|----------|------|
| MAJOR | 구조 재편 또는 대규모 재작성 |
| MINOR | 새 섹션, 중요한 내용 추가 |
| PATCH | 사소한 교정, 오탈자 수정 |

**중요**: 문서 개정 버전(문서 이력 표에서 추적)은 제품 버전(문서 헤더에 표시)과 별개입니다.

#### 문서 간 버전 참조

다른 문서를 참조할 때는 항상 참조 중인 특정 버전을 명시합니다:

| 문서 | 현재 버전 | 권한 범위 |
|------|----------|----------|
| PRD | 0.2.0.0 | 단계 정의, 기능 요구사항 |
| SRS | 0.1.3.1 | 상세 요구사항 명세 |
| SDS | 0.2.0.0 | 소프트웨어 설계, 컴포넌트 아키텍처 |
| FEATURES | 0.2.0.0 | 기능 문서 및 예시 |
...
| 2.1.0 | 2026-02-09 | raphaelshin | 추가: 단계 요약 표, 버전 번호 규약 (부록 C). 수정: 교차 참조의 단계 목표 정렬 (Issue #673). |
| 2.2.0 | 2026-02-09 | raphaelshin | 추가: IR-1 에코시스템 통합 테이블에 database_system 추가. 수정: 에코시스템 카운트 6 → 7개 시스템, 이슈 #674. |
