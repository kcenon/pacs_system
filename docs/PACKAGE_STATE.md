...
(`NetworkSystem`, `LoggerSystem`). `cmake/dependencies.cmake` accepts those names after the
current `network_system`/`logger_system` names, so `cmake --preset vcpkg` works with the
reviewed registry baseline.

## Dependency provenance

`dependency-manifest.json` records the current FetchContent pins (SQLite
`sqlite-amalgamation-3450300` / 3.45.3, OpenJPH 0.21.0, Crow v1.3.1, ASIO `asio-1-30-2`) and the
ecosystem source tuple validated by
...
These Git SHAs are build-provenance pins, not published version constraints. Published v1.x
dependency constraints are owned by #1164 and replace these pins only after every Tier 0-4
dependency ships v1.0.
