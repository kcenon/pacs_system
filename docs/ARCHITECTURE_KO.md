...
| 컴포넌트 | 버전 | 비고 |
|----------|------|------|
| CMake | 3.20+ | 빌드 시스템 생성기 |
| C++ 표준 | C++20 | concepts, ranges, std::format 필요 |
| GCC | 13+ | Ubuntu 24.04+에서 GCC 13의 std::format 제공 (logger_system 필요) |
| Clang/Apple Clang | 15+ | macOS 14+에서 Apple Clang 15 제공 |
...
│  ┌───────────────────────────────────────────────────────────┐  │
│  │                  워커 스레드 풀                             │  │
│  │            (integration::thread_adapter 싱글톤)            │  │
│  │                                                            │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │  │
│  │  │ Worker 1 │  │ Worker 2 │  │ Worker 3 │  │ Worker N │  │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘  │  │
│  │                                                            │  │
│  │  • Fire-and-forget 태스크 제출                            │  │
...
│  │                      │  │                      │  │    (Optional)       │  │
│  │  • 비동기 로깅        │  │  • 성능 메트릭        │  │  • Query builder    │  │
│  │  • 감사 추적          │  │  • 상태 확인          │  │  • SQL injection    │  │
│  │  • 다중 라이터        │  │  • 분산 추적          │  │    protection       │  │
│  │  • HIPAA 준수         │  │                      │  │  • Parameterized    │  │
│  └──────────────────────┘  └──────────────────────┘  └──────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
DICOM 서버는 효율적인 태스크 스케줄링과 수명 주기 관리를 위해 에코시스템의
`thread_system`을 활용하는 다중 계층 스레딩 모델을 사용합니다:

- **Accept Worker**: 연결 수락을 위해 `thread_base`를 상속하며
  적절한 수명 주기 관리(시작/중지, 취소 토큰)를 제공합니다.
- **Association Workers**: DIMSE 메시지 처리를 위해 `thread_adapter::submit_fire_and_forget()`를
  사용하여 연결별 스레드 관리 오버헤드를 제거합니다.
...
│  │              (detail::accept_worker → thread_base)                      │ │
│  │                                                                         │ │
│  │  • 설정된 포트에서 수신 대기 (기본값 11112)                             │ │
│  │  • 주기적 유지보수 콜백 (유휴 타임아웃 체크)                            │ │
│  │  • thread_base 수명주기를 통한 정상 종료                                │ │
│  │  • Wake interval 설정 가능 (기본값 100ms)                               │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                      │                                       │
│                                      │ 새 연결                               │
│                                      ▼                                       │
...
│  │                                                                         │ │
│  │  주요 특징:                                                             │ │
│  │  • Fire-and-forget 태스크 제출 (스레드 join 불필요)                    │ │
│  │  • 우선순위 스케줄링: critical > high > normal > low                   │ │
│  │  • 연결 간 스레드 재사용                                                │ │
│  │  • 락프리 작업 큐 (thread_system::job_queue)                           │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                      저장 스레드 풀                                     │ │
...
| 버전 | 날짜 | 작성자 | 변경 사항 |
|------|------|--------|----------|
| 0.1.0.0 | 2025-11-30 | kcenon | 초기 작성 |
| 0.1.4.0 | 2026-02-09 | raphaelshin | 추가: 빌드 요구사항 섹션, 에코시스템 통합에 database_system 추가. 명확화: 모듈 전용 vs 기본 빌드 요구사항, 이슈 #674. |
