...
TEST_CASE("jpeg_baseline_codec grayscale round-trip", "[encoding][compression]") {
    ...
    SECTION("encode succeeds") {
        ...
        REQUIRE(encode_result.value().data.size() < original.size());  // Should compress
    }
    ...
    SECTION("round-trip maintains quality") {
        ...
        double psnr = calculate_psnr(original, decode_result.value().data);
...
TEST_CASE("jpeg_baseline_codec color round-trip", "[encoding][compression]") {
    ...
    SECTION("output params are set correctly for color") {
        ...
        auto decode_result = codec.decode(encode_result.value().data, params);
        REQUIRE(decode_result.is_ok() == true);
        REQUIRE(decode_result.value().output_params.samples_per_pixel == 3);
        REQUIRE(decode_result.value().output_params.photometric == photometric_interpretation::rgb);
    }
}
...
TEST_CASE("jpeg_baseline_codec quality settings", "[encoding][compression]") {
    ...
    SECTION("lower quality produces smaller files") {
        ...
        REQUIRE(low_result.value().data.size() < high_result.value().data.size());
    }
...
TEST_CASE("codec_factory creates jpeg_baseline_codec", "[encoding][compression]") {
    ...
    SECTION("invalid UID returns nullptr") {
        auto codec = codec_factory::create("1.2.3.4.5.invalid.uid");
        REQUIRE(codec == nullptr);
    }
