...
TEST_CASE("Annotation repository operations", "[web][annotation][database]") {
  ...
  // Create pacs_database_adapter for base_repository version
  auto db_adapter = std::make_shared<pacs_database_adapter>(":memory:");
  REQUIRE(db_adapter->connect().is_ok());

  annotation_repository repo(db_adapter);
  ...
  SECTION("save and find annotation") {
    ...
    REQUIRE(found.is_ok());
    REQUIRE(found.value().annotation_id == "test-uuid-123");
  ...
  SECTION("find by instance") {
    ...
    REQUIRE(annotations.is_ok());
    REQUIRE(annotations.value().size() == 2);
  }
  ...
  SECTION("delete annotation") {
    ...
    auto exists_after = repo.exists("delete-test");
