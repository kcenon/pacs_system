...
PerformanceTracker::~PerformanceTracker() {
    ...
    if (m_trackingEnabled) {
        log_module::write_information("Final Performance Statistics: %s", getStatisticsJson().c_str());
    }
}

void PerformanceTracker::initialize(bool enableTracking) {
    m_trackingEnabled = enableTracking;
    log_module::write_information("Performance tracking %s", enableTracking ? "enabled" : "disabled");
}
