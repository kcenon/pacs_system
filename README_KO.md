...
| **Phase 5**: Enterprise Features | VTK Integration, FHIR, Clustering | 🔜 예정 |

**테스트 커버리지**: 141개 이상 테스트 파일, 1,980개 이상 테스트 통과

> Phase별 상세 기능 목록은 [기능 명세](docs/FEATURES_KO.md)를 참조하세요.
...
> 모든 옵션을 포함한 전체 CLI 문서는 [CLI 레퍼런스](docs/CLI_REFERENCE_KO.md)를 참조하세요.
...
| **Integration** | `include/pacs/integration/` | 에코시스템 어댑터 (container, network, thread, logger, monitoring, ITK) |
...
> 전체 파일 수준 디렉토리 트리는 [프로젝트 구조](docs/PROJECT_STRUCTURE_KO.md)를 참조하세요.
...
- 🖥️ [CLI 레퍼런스](docs/CLI_REFERENCE_KO.md) - CLI 도구 문서
...
상세 벤치마크 결과는 [PERFORMANCE_RESULTS.md](docs/PERFORMANCE_RESULTS.md), 마이그레이션 요약은 [MIGRATION_COMPLETE_KO.md](docs/MIGRATION_COMPLETE_KO.md)를 참조하세요.
