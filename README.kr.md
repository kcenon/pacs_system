...
v1.0을 게시한 뒤(#1164)에 출시됩니다. 계약 문서는 `develop` 브랜치에서 관리됩니다. 검토 중인 헤더 목록은
[`docs/v1.0-api-surface.md`](https://github.com/kcenon/pacs_system/blob/develop/docs/v1.0-api-surface.md)를, 계획된 예외/Result<T> 계약은
[`docs/v1.0-throw-policy.md`](https://github.com/kcenon/pacs_system/blob/develop/docs/v1.0-throw-policy.md)를, 고정 이전 지원 중단 감사는
[`docs/v1.0-deprecation-inventory.md`](https://github.com/kcenon/pacs_system/blob/develop/docs/v1.0-deprecation-inventory.md)를 참조하세요.

v1.0을 준비하시나요? [0.x → 1.0 마이그레이션 가이드](https://github.com/kcenon/pacs_system/blob/develop/docs/migration/0.x-to-1.0.md)는
계획된 업그레이드 절차(include 경로, 네임스페이스, CMake 계약, `Result<T>` 마이그레이션, 디렉터리 재배치)를
설명하며, `samples/` → `examples/` 및 `examples/` → `tools/` 디렉터리 재배치(#1139)와
`pacs_system::pacs_system` CMake 계약(#1158)도 포함합니다. [CHANGELOG](CHANGELOG.md)는 0.1.0 이후의
모든 작업을 `[Unreleased]` 아래에 두며, v1.0.0 릴리스가 실제로 태그되기 전까지 v1.0.0 섹션을 만들지 않습니다.
...
아직 어떤 게시 패키지에도 포함되지 않았으며, v1.0.0 릴리스가 출시된 후 다운스트림 프로젝트는 다음과 같이 의존하게 됩니다:
