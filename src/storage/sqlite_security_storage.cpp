...
sqlite_security_storage::sqlite_security_storage(std::string db_path)
  ...
  if (auto res = initialize_database(); res.is_err()) {
...
auto sqlite_security_storage::initialize_database() -> VoidResult {
...
auto sqlite_security_storage::update_user(const User &user) -> VoidResult {
  ...
  // Update user using query_builder
  ...
  // Delete existing roles using query_builder
  ...
  // Insert new roles using query_builder
  for (const auto &role : user.roles) {
    database::query_builder role_builder(database::database_types::sqlite);
    auto role_sql =
...
auto sqlite_security_storage::get_users_by_role(Role role)
  ...
  for (const auto &row : result.value()) {
    ...
    // Fetch all roles for this user using query_builder
    database::query_builder role_builder(database::database_types::sqlite);
    auto role_sql =
