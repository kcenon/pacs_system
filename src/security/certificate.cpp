 ...
 *
 * Uses an explicit size-then-read pattern instead of streaming via
 * `std::ostringstream` and `rdbuf()`. The streaming form has a portability
 * pitfall on macOS with Apple Clang's libc++: when the input is a binary
 * stream and the target is a different stream type, intermediate buffer
 * boundaries can interact poorly with `std::sentry` and silently truncate
 * the resulting string for files that lack a trailing newline (commonly
 * the case for PEM `.key` files generated by OpenSSL on macOS where the
 * final line is the END marker without a trailing `\n`). Reading by
 * known length with `std::ifstream::read` avoids the streambuf round-trip
 * and produces identical bytes on Linux, Windows, and macOS.
 *
 * Also rejects paths that are not regular files (directories, broken
 * symlinks, FIFOs) so callers get a clear error rather than an empty
 * buffer that later fails with a confusing OpenSSL parse error.
 */
auto read_file(std::string_view path) -> kcenon::common::Result<std::string> {
    const std::string path_str(path);
    std::error_code ec;

    // Validate path is a regular file. On macOS, attempting to open a
    // symlink whose target was removed (a frequent failure mode for test
    // fixtures regenerated on each CI run) returns an opened ifstream
    // whose subsequent reads are empty, masking the real error.
    if (!std::filesystem::is_regular_file(path_str, ec) || ec) {
        return kcenon::common::make_error<std::string>(
            1, "Failed to open file (not a regular file): " + path_str,
            "certificate");
    }
    ...
    if (ec) {
        return kcenon::common::make_error<std::string>(
            1, "Failed to stat file: " + path_str, "certificate");
    }

    std::ifstream file(path_str, std::ios::binary);
    if (!file.is_open()) {
        return kcenon::common::make_error<std::string>(
            1, "Failed to open file: " + path_str, "certificate");
    }
    ...
    if (size > 0) {
        ...
        if (!file) {
            return kcenon::common::make_error<std::string>(
                1, "Failed to read file: " + path_str, "certificate");
        }
    }
    return content;
}
