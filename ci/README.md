...
```sh
python3 scripts/check_tag_reality.py --repo kcenon/pacs_system --tag v1.2.3 \
  --release-commit FULL_RELEASE_MERGE_SHA \
  --port-dir vcpkg-ports/kcenon-pacs-system
```
...
- `PACS_WITH_COMMON_SYSTEM` → `KCENON_WITH_COMMON_SYSTEM`.
- `PACS_WITH_CONTAINER_SYSTEM` → `KCENON_WITH_CONTAINER_SYSTEM`.
- `PACS_WITH_NETWORK_SYSTEM` → `KCENON_WITH_NETWORK_SYSTEM`.
