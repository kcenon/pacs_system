...
# pugixml (for IHE XDS.b SOAP envelope construction - Issue #1128)
message(STATUS "")
message(STATUS "=== Finding pugixml (for IHE XDS.b SOAP envelopes) ===")
find_package(pugixml CONFIG QUIET)
if(NOT pugixml_FOUND)
    find_package(PugiXML QUIET)
endif()
if(pugixml_FOUND OR PugiXML_FOUND)
    set(PACS_PUGIXML_FOUND TRUE)
    message(STATUS "Found pugixml")
else()
    set(PACS_PUGIXML_FOUND FALSE)
    message(STATUS "pugixml not found - IHE XDS.b Document Source (ITI-41) will be disabled")
    message(STATUS "  Install with: vcpkg install pugixml (or: brew install pugixml / apt install libpugixml-dev)")
endif()

# libcurl (for IHE XDS.b HTTPS transport - Issue #1128)
message(STATUS "")
message(STATUS "=== Finding libcurl (for IHE XDS.b HTTPS transport) ===")
find_package(CURL CONFIG QUIET)
if(NOT CURL_FOUND)
    find_package(CURL QUIET)
endif()
if(CURL_FOUND)
    set(PACS_CURL_FOUND TRUE)
    ...
    set(PACS_CURL_FOUND FALSE)
    message(STATUS "libcurl not found - IHE XDS.b Document Source (ITI-41) will be disabled")
    message(STATUS "  Install with: vcpkg install curl (or: brew install curl / apt install libcurl4-openssl-dev)")
endif()
...
set(PACS_SOUP_THREAD_SYSTEM_SHA     "db8d36f15cf2dfb679474b20cb47160494a6c8be")
set(PACS_SOUP_LOGGER_SYSTEM_SHA     "f119ecd698fb5a8b25dd730647b9d038034aad5a")
set(PACS_SOUP_MONITORING_SYSTEM_SHA "221bf3d2645a2bfd54b07fd84b1c24c2e32d754f")
set(PACS_SOUP_NETWORK_SYSTEM_SHA    "093691d9523265d672880e2cb7c0ea0e4463a8f8")
...
if(PACS_WITH_COMMON_SYSTEM)
    ...
    if(TARGET kcenon::common_system)
        ...
        message(STATUS "Using installed common_system target")
    else()
...
if(PACS_WITH_CONTAINER_SYSTEM)
    ...
    if(TARGET ContainerSystem::container OR TARGET container_system::container_system)
        if(NOT TARGET container_system)
            ...
            if(TARGET ContainerSystem::container)
                ...
                target_link_libraries(container_system INTERFACE container_system::container_system)
            endif()
        endif()
        set(PACS_CONTAINER_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        set(CONTAINER_SYSTEM_FOUND TRUE)
        message(STATUS "Using installed container_system target")
    else()
        foreach(_path ${_PACS_CONTAINER_PATHS})
            if(EXISTS "${_path}/CMakeLists.txt" AND EXISTS "${_path}/include/kcenon/container/container.h")
    ...
    if(NOT CONTAINER_SYSTEM_FOUND AND NOT TARGET container_system)
        ...
        if(container_system_FOUND)
            message(STATUS "Found container_system via find_package(container_system)")
            set(CONTAINER_SYSTEM_FOUND TRUE)
            if(NOT TARGET container_system)
                add_library(container_system INTERFACE)
                target_link_libraries(container_system INTERFACE ContainerSystem::container)
    ...
    # Published registry ports (container_system 0.1.x) install the legacy ContainerSystem package name
    if(NOT CONTAINER_SYSTEM_FOUND AND NOT TARGET container_system)
        find_package(ContainerSystem CONFIG QUIET)
        if(ContainerSystem_FOUND AND TARGET ContainerSystem::container)
            message(STATUS "Found container_system via find_package(ContainerSystem) ${ContainerSystem_VERSION}")
            set(CONTAINER_SYSTEM_FOUND TRUE)
            set(PACS_CONTAINER_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
            add_library(container_system INTERFACE)
            target_link_libraries(container_system INTERFACE ContainerSystem::container)
...
if(TARGET thread_system::thread_system OR TARGET thread_system::thread_base)
    if(NOT TARGET thread_system)
        add_library(thread_system INTERFACE)
        if(TARGET thread_system::thread_system)
            target_link_libraries(thread_system INTERFACE thread_system::thread_system)
        else()
            target_link_libraries(thread_system INTERFACE thread_system::thread_base)
        endif()
    endif()
    set(PACS_THREAD_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
    set(THREAD_SYSTEM_FOUND TRUE)
    message(STATUS "Using installed thread_system target")
else()
...
# find_package() fallback for vcpkg/system installs
if(NOT THREAD_SYSTEM_FOUND AND NOT TARGET thread_base AND NOT TARGET thread_system)
    find_package(thread_system CONFIG QUIET)
    if(thread_system_FOUND)
        message(STATUS "Found thread_system via find_package(thread_system)")
        set(THREAD_SYSTEM_FOUND TRUE)
        set(PACS_THREAD_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        # Create non-namespaced target for compatibility with downstream checks
        if(NOT TARGET thread_system AND TARGET thread_system::thread_base)
            add_library(thread_system INTERFACE)
            target_link_libraries(thread_system INTERFACE thread_system::thread_base)
        endif()
    endif()
...
if(TARGET thread_base OR TARGET thread_system OR TARGET interfaces)
...
# Installed-package target used by the pacs_system export bridge
set(PACS_LOGGER_SYSTEM_INSTALL_TARGET logger_system::logger_system)
if(TARGET logger_system::logger OR TARGET logger_system::logger_system)
    if(NOT TARGET logger_system)
        add_library(logger_system INTERFACE)
        if(TARGET logger_system::logger)
            target_link_libraries(logger_system INTERFACE logger_system::logger)
            set(PACS_LOGGER_SYSTEM_INSTALL_TARGET logger_system::logger)
        else()
            target_link_libraries(logger_system INTERFACE logger_system::logger_system)
        endif()
    endif()
    set(PACS_LOGGER_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
    set(LOGGER_SYSTEM_FOUND TRUE)
    message(STATUS "Using installed logger_system target")
else()
...
# find_package() fallback for vcpkg/system installs
if(NOT LOGGER_SYSTEM_FOUND AND NOT TARGET logger_system)
    find_package(logger_system CONFIG QUIET)
    if(logger_system_FOUND)
        message(STATUS "Found logger_system via find_package(logger_system)")
        set(LOGGER_SYSTEM_FOUND TRUE)
        set(PACS_LOGGER_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        # Create non-namespaced target for compatibility with downstream checks
        if(NOT TARGET logger_system)
            if(TARGET logger_system::logger_system)
                add_library(logger_system INTERFACE)
                target_link_libraries(logger_system INTERFACE logger_system::logger_system)
            elseif(TARGET logger_system::logger)
                add_library(logger_system INTERFACE)
                target_link_libraries(logger_system INTERFACE logger_system::logger)
            endif()
        endif()
...
# Published registry ports (logger_system 0.1.x) install the legacy LoggerSystem package name
if(NOT LOGGER_SYSTEM_FOUND AND NOT TARGET logger_system)
    find_package(LoggerSystem CONFIG QUIET)
    if(LoggerSystem_FOUND AND TARGET LoggerSystem::logger)
        message(STATUS "Found logger_system via find_package(LoggerSystem) ${LoggerSystem_VERSION}")
        set(LOGGER_SYSTEM_FOUND TRUE)
        set(PACS_LOGGER_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        add_library(logger_system INTERFACE)
        target_link_libraries(logger_system INTERFACE LoggerSystem::logger)
        set(PACS_LOGGER_SYSTEM_INSTALL_TARGET LoggerSystem::logger)
    endif()
endif()

if(LOGGER_SYSTEM_FOUND AND NOT TARGET logger_system)
    set(BUILD_SAMPLES OFF CACHE BOOL "" FORCE)
    set(BUILD_TESTS OFF CACHE BOOL "" FORCE)
...
elseif(NOT LOGGER_SYSTEM_FOUND AND NOT TARGET logger_system)
    message(WARNING
...
if(PACS_BUILD_STORAGE)
    ...
    if(TARGET database_system::database_system)
        if(NOT TARGET database)
            add_library(database INTERFACE)
            target_link_libraries(database INTERFACE database_system::database_system)
        endif()
        if(TARGET database_system::integrated_database AND NOT TARGET integrated_database)
            add_library(integrated_database INTERFACE)
            target_link_libraries(integrated_database INTERFACE database_system::integrated_database)
        endif()
        set(PACS_DATABASE_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        set(DATABASE_SYSTEM_FOUND TRUE)
        message(STATUS "Using installed database_system target")
    else()
        foreach(_path ${_PACS_DATABASE_PATHS})
            if(EXISTS "${_path}/CMakeLists.txt" AND EXISTS "${_path}/include/kcenon/database/database_manager.h")
    ...
    if(NOT DATABASE_SYSTEM_FOUND AND NOT TARGET database)
        find_package(database_system CONFIG QUIET)
        if(database_system_FOUND)
            message(STATUS "Found database_system via find_package(database_system)")
            set(DATABASE_SYSTEM_FOUND TRUE)
            if(NOT TARGET database AND TARGET database_system::database_system)
                add_library(database INTERFACE)
                target_link_libraries(database INTERFACE database_system::database_system)
            endif()
        endif()
...
if(PACS_BUILD_STORAGE)
    ...
    if(TARGET monitoring_system::monitoring_system)
        if(NOT TARGET monitoring_system)
        ...
        message(STATUS "Using installed monitoring_system target")
    else()
...
if(PACS_WITH_NETWORK_SYSTEM)
    ...
    # Installed-package target used by the pacs_system export bridge
    set(PACS_NETWORK_SYSTEM_INSTALL_TARGET network_system::network_system)
    if(TARGET network_system::network_system)
        if(NOT TARGET network_system)
            add_library(network_system INTERFACE)
            target_link_libraries(network_system INTERFACE network_system::network_system)
        endif()
        set(NETWORK_SYSTEM_FOUND TRUE)
        set(PACS_NETWORK_SYSTEM_INCLUDE_DIR "FOUND_VIA_PACKAGE")
        message(STATUS "Using installed network_system target")
    else()
    ...
    # find_package() fallback for vcpkg/system installs
    if(NOT NETWORK_SYSTEM_FOUND AND NOT TARGET network_system)
        find_package(network_system CONFIG QUIET)
        if(network_system_FOUND)
            message(STATUS "Found network_system via find_package(network_system)")
            set(NETWORK_SYSTEM_FOUND TRUE)
            # Create non-namespaced target for compatibility with downstream checks
            if(NOT TARGET network_system AND TARGET network_system::network_system)
                add_library(network_system INTERFACE)
                target_link_libraries(network_system INTERFACE network_system::network_system)
            endif()
        endif()
    ...
    # Published registry ports (network_system 0.1.x) install the legacy NetworkSystem package name
    if(NOT NETWORK_SYSTEM_FOUND AND NOT TARGET network_system)
        find_package(NetworkSystem CONFIG QUIET)
        if(NetworkSystem_FOUND AND TARGET NetworkSystem::NetworkSystem)
            message(STATUS "Found network_system via find_package(NetworkSystem) ${NetworkSystem_VERSION}")
            set(NETWORK_SYSTEM_FOUND TRUE)
            add_library(network_system INTERFACE)
            target_link_libraries(network_system INTERFACE NetworkSystem::NetworkSystem)
            set(PACS_NETWORK_SYSTEM_INSTALL_TARGET NetworkSystem::NetworkSystem)
        endif()
    endif()

    if(NETWORK_SYSTEM_FOUND AND NOT TARGET network_system)
        set(BUILD_SAMPLES OFF CACHE BOOL "" FORCE)
        set(BUILD_TESTS OFF CACHE BOOL "" FORCE)
        ...
        endforeach()
    elseif(NOT NETWORK_SYSTEM_FOUND AND NOT TARGET network_system)
        message(FATAL_ERROR
